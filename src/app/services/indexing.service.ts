...
export class IndexingService {
  ...
  public async indexFile(filePath: string): Promise<Comic | null> {
    try {
      ...
      try {
        console.log(`Extracting cover from: ${fileInfo.name}`);
        coverData = await this.cbzService.extractCover(filePath);
        console.log(`Cover extraction result:`, coverData);
        
        if (coverData.cover && coverData.mimeType) {
          hasCover = true;
          coverImageUrl = this.cbzService.getCoverUrl(coverData.cover, coverData.mimeType);
          console.log(`✓ Extracted cover for: ${fileInfo.name}`);
        }

        // Get total pages
        console.log(`Getting page count for: ${fileInfo.name}`);
        const fullData = await this.cbzService.readCBZ(filePath);
        totalPages = fullData.pages.length;
        console.log(`✓ Found ${totalPages} pages in ${fileInfo.name}`);
      } catch (error) {
        console.warn(`Failed to fully index ${fileInfo.name}:`, error);
      }
      ...
      console.log(`Created comic object:`, comic);

      this.storageService.addComic(comic);
      console.log(`✓ Successfully indexed: ${comic.title}`);
